...
| 2026-02-05 | Phase 7 Step 1 | 清算エンジンDB設計 + API実装（5テーブル + 7 RPC + 8 API Routes） | 作業中 |
...
- **RPC関数（Migration 017）**: `generate_settlement_entries` が複数月まとめ清算・後出しレシート・末日対応を一括処理。`confirm_settlement` が原子的に payments/payment_splits に変換
...
- `feature/phase7-settlement-engine` — Phase 7 清算エンジン実装中（DB設計 + API完了、次はUI）
...
- [ ] Step 2: 固定費ルール設定UI — `/groups/[id]/recurring-rules` ページ
- [ ] Step 3: 清算準備室UI — `/groups/[id]/settlement` ページ
- [ ] Step 4: チェックリスト入力UI — 金額入力・ステータス変更
- [ ] Step 5: 確定処理 & 清算済み表示
